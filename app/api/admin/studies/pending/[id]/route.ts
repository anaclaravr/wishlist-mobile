...
import { deleteStudyPendingItem, updateStudyPendingItem } from "@/lib/access-db";
...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const pendingItem = await updateStudyPendingItem({
      id,
...
export async function DELETE(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const session = await requireAccessSession({ role: "admin", permission: "admin.studies.manage" });
    const { id } = await context.params;
    await deleteStudyPendingItem({ id, wishlistId: session.wishlistId });
