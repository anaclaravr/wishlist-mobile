...
import { createStudyCourse, listStudyData } from "@/lib/access-db";
...
export async function GET() {
  try {
    ...
    const data = await listStudyData({ wishlistId: session.wishlistId });
...
export async function POST(request: Request) {
  try {
    ...
    const course = await createStudyCourse({
